-- Jalankan file ini SETELAH MIGRASI_modul_dagang_v13a_enum.sql berhasil.
...
-- ----------------------------------------------------------------------------
-- PENTING: pakai DROP dulu (bukan cuma CREATE OR REPLACE) karena kolom hasil
-- fungsi ini berubah (nambah kolom receipt_path) — PostgreSQL gak izinin
-- CREATE OR REPLACE FUNCTION mengubah tipe/struktur kolom RETURNS TABLE yang
-- sudah ada, cuma boleh kalau strukturnya persis sama.
drop function if exists get_transaction_report(uuid, timestamptz, timestamptz, uuid, uuid, uuid, uuid);
