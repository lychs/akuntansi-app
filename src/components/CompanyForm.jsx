...
const LOCATIONS = ['Indonesia', 'Malaysia', 'Singapura', 'Amerika Serikat', 'Inggris', 'Australia', 'Jepang', 'Tiongkok', 'Lainnya']
const CURRENCIES = ['IDR', 'USD', 'EUR', 'SGD', 'MYR', 'GBP', 'AUD', 'JPY', 'CNY']
...
export default function CompanyForm({ onSuccess, submitLabel, mode = 'create', initialData = null }) {
  ...
  const [financialYearStartMonth, setFinancialYearStartMonth] = useState(initialData?.financial_year_start_month || 1)
  const [bookkeepingStartDate, setBookkeepingStartDate] = useState(initialData?.bookkeeping_start_date || '')
  const [businessLocation, setBusinessLocation] = useState(initialData?.business_location || 'Indonesia')
  const [taxId, setTaxId] = useState(initialData?.tax_id || '')
  const [baseCurrency, setBaseCurrency] = useState(initialData?.base_currency || 'IDR')
  ...
  const BUSINESS_TYPES = [
    ...
    { value: 'manufaktur', label: t('companyForm.businessTypeManufaktur'), desc: t('companyForm.businessTypeManufakturDesc'), icon: Factory, disabled: false },
  ]
    ...
    try {
      ...
      if (mode === 'edit') {
        const { data, error } = await supabase.from('companies').update({
          ...
          bookkeeping_start_date: bookkeepingStartDate || null,
        ...
        if ((businessType === 'dagang' || businessType === 'manufaktur') && !inventoryMethod) {
        ...
        const { data, error } = await supabase.rpc('create_company', {
          ...
          p_inventory_method: (businessType === 'dagang' || businessType === 'manufaktur') ? inventoryMethod : null,
        })
        if (error) throw error
        onSuccess(data)
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="company-form">
      ...
      {mode === 'edit' && (
        <>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '24px 0 4px' }}>{t('companyForm.advancedSettingsTitle')}</h3>
          <p className="hint" style={{ marginTop: 0, marginBottom: 14 }}>{t('companyForm.advancedSettingsDesc')}</p>

          <div className="company-form-row-2col">
            <div>
              <label>{t('companyForm.financialYearLabel')}</label>
              <select value={financialYearStartMonth} onChange={(e) => setFinancialYearStartMonth(Number(e.target.value))}>
                {MONTHS_ID.map((m, idx) => <option key={idx} value={idx + 1}>{t('companyForm.financialYearStartsAt', { month: m })}</option>)}
              </select>
              <p className="hint" style={{ marginTop: 4 }}>{t('companyForm.financialYearHint')}</p>
            </div>
            <div>
              <label>{t('companyForm.bookkeepingStartLabel')}</label>
              <input type="date" value={bookkeepingStartDate} onChange={(e) => setBookkeepingStartDate(e.target.value)} />
              <p className="hint" style={{ marginTop: 4 }}>{t('companyForm.bookkeepingStartHint')}</p>
            </div>
          </div>

          <div className="company-form-row-2col">
            <div>
              <label>{t('companyForm.locationLabel')}</label>
              <select value={businessLocation} onChange={(e) => setBusinessLocation(e.target.value)}>
                {LOCATIONS.map((loc) => <option key={loc} value={loc}>{loc}</option>)}
              </select>
            </div>
            <div>
              <label>{businessLocation === 'Indonesia' ? t('companyForm.npwpLabel') : t('companyForm.taxIdLabel')}</label>
              <input value={taxId} onChange={(e) => setTaxId(e.target.value)} placeholder={businessLocation === 'Indonesia' ? '00.000.000.0-000.000' : ''} />
            </div>
          </div>

          <div className="company-form-row-2col">
            <div>
              <label>{t('companyForm.baseCurrencyLabel')}</label>
              <select value={baseCurrency} onChange={(e) => setBaseCurrency(e.target.value)}>
                {CURRENCIES.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
              <p className="hint" style={{ marginTop: 4 }}>{t('companyForm.baseCurrencyHint')}</p>
            </div>
            <div>
              <label>{t('companyForm.companyPhoneLabel')}</label>
              <input value={companyPhone} onChange={(e) => setCompanyPhone(e.target.value)} placeholder="+62 812-3456-7890" />
            </div>
          </div>

          <label>{t('companyForm.companyEmailLabel')}</label>
          <input type="email" value={companyEmail} onChange={(e) => setCompanyEmail(e.target.value)} placeholder="info@perusahaan.com" />
        </>
      )}
      ...
      {mode === 'create' && (
        <>
          ...
          {(businessType === 'dagang' || businessType === 'manufaktur') && (
            <>
