...
import { Paperclip, X, FileText, Image as ImageIcon, Lock } from 'lucide-react'
...
// Dikunci buat paket Free — nge-cek activePlan di sini SEKALI, otomatis
// kepakai di semua form yang make komponen ini.
export default function ReceiptUpload({ file, setFile }) {
  const { t } = useTranslation()
  const { activePlan } = useCompany()
  ...
  if (activePlan === 'free') {
    return (
      <div className="field">
        ...
        <Link to="/billing" className="btn-secondary receipt-upload-btn" style={{ color: 'var(--ink-muted)', textDecoration: 'none' }}>
          <Lock size={14} style={{ verticalAlign: -2, marginRight: 6 }} />
          {t('receiptUpload.lockedFree')}
        </Link>
      </div>
    )
  }
