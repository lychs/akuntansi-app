...
export function CompanyProvider({ children }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('company_users')
      .select('company_id, role, companies ( id, name, logo_url, business_type, inventory_method, plan, hidden_menu_keys, trial_expires_at, plan_expires_at )')
      .eq('user_id', userId)
    if (error) {
      console.error(error)
      setLoading(false)
    ...
    for (const r of data || []) {
      ...
      list.push({
        ...
        plan: r.companies.plan, hiddenMenuKeys: r.companies.hidden_menu_keys || [],
        trialExpiresAt: r.companies.trial_expires_at, planExpiresAt: r.companies.plan_expires_at,
      })
    }
    setCompanies(list)
    setLoading(false)
  ...
  const activeHiddenMenuKeys = companies.find((c) => c.id === activeCompanyId)?.hiddenMenuKeys || []
  const activePlan = companies.find((c) => c.id === activeCompanyId)?.plan || 'free'
  const activeTrialExpiresAt = companies.find((c) => c.id === activeCompanyId)?.trialExpiresAt || null
  const activePlanExpiresAt = companies.find((c) => c.id === activeCompanyId)?.planExpiresAt || null

  return (
    <CompanyContext.Provider value={{ companies, activeCompanyId, setActiveCompanyId, loading, refresh, canEdit, activeRole, activeBusinessType, activeHiddenMenuKeys, activePlan, activeTrialExpiresAt, activePlanExpiresAt }}>
      {children}
