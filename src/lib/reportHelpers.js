...
// Arah "normal" tiap kategori akun — dipakai buat ngenalin akun KONTRA (misal
// "Retur Penjualan" = kategori pendapatan tapi normal_balance-nya DEBIT, bukan
// KREDIT kayak akun pendapatan pada umumnya). Akun kontra harus MENGURANGI
// total kategorinya, bukan menambah.
const CATEGORY_NORMAL_DIRECTION = {
  pendapatan: 'kredit',
  beban_pokok: 'debit',
...
export function sumCategory(rows, category) {
  const expectedDirection = CATEGORY_NORMAL_DIRECTION[category]
  return (rows || []).filter((r) => r.category === category).reduce((s, r) => {
    const amt = netAmount(r)
    // Akun yang normal_balance-nya BEDA dari arah normal kategorinya adalah
    // akun kontra — balikin tandanya.
    const isContra = expectedDirection && r.normal_balance !== expectedDirection
    return s + (isContra ? -amt : amt)
  }, 0)
}
