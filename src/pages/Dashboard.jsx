...
import { TrendingUp, TrendingDown, Wallet, HandCoins, Clock3, ArrowUpRight, ArrowDownRight, Percent, Package, Repeat } from 'lucide-react'
...
export default function Dashboard() {
  ...
  async function loadAll(cid) {
    ...
    supabase.rpc('get_due_recurring_transactions', { p_company_id: cid }).then(({ data }) => setRecurringDue(data || []))
    ...
    // Tagihan jatuh tempo (Piutang & Hutang yang belum lunas)
  ...
  return (
    <div className="p-6">
      {recurringDue.length > 0 && (
        <Link to="/transaksi/berulang" className="recurring-due-banner">
          <Repeat size={16} />
          <span>{t('dashboard.recurringDueBanner', { count: recurringDue.length })}</span>
        </Link>
      )}
      ...
      <div className="dash-grid-bottom">
        <div className="section">
          ...
          <table className="tbl compact">
            <thead><tr><th>{t('dashboard.colDueDate')}</th><th>{t('dashboard.colType')}</th><th>{t('dashboard.colCustomer')}</th><th className="num">{t('dashboard.colAmount')}</th><th>{t('dashboard.colDaysLeft')}</th></tr></thead>
            <tbody>
              {dueSoon.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    <td>{r.due_date}</td>
                    <td><span className={`badge ${r.type === 'hutang' ? 'overdue' : 'pending'}`}>{r.type === 'hutang' ? t('hutangPiutang.typeHutang') : t('hutangPiutang.typePiutang')}</span></td>
              ...
              {!dueSoon.length && <tr><td colSpan={5} className="empty">{t('dashboard.noDue')}</td></tr>}
            </tbody>
          </table>
        </div>
