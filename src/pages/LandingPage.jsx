  ...
  return (
    <div className={`lp ${isDark ? 'dark' : ''}`}>
      ...
      <section className="lp-section lp-section-alt">
        <div className="lp-wrap">
          <Reveal className="lp-center" style={{ maxWidth: 480, margin: '0 auto 40px' }}>
            <span className="lp-eyebrow-label">{t('changelog.kicker')}</span>
            <h2 className="lp-h2 lp-head">{t('changelog.title')}</h2>
            <p className="lp-p-lead lp-center" style={{ margin: '0 auto' }}>{t('changelog.subtitle')}</p>
          </Reveal>
          <Reveal className="lp-changelog-list">
            {t('changelog.items', { returnObjects: true }).map((item, i) => (
              <div className="lp-changelog-item" key={i}>
                <span className="lp-changelog-num">{i + 1}</span>
                <p className="lp-changelog-text">{item}</p>
              </div>
            ))}
          </Reveal>
        </div>
