...
const ITEM_TYPES = ['raw_material', 'supporting_material', 'wip', 'finished_goods', 'scrap']

export default function Produk() {
  const { t, i18n } = useTranslation()
  const companyId = localStorage.getItem('activeCompanyId')
  const { canEdit, activeBusinessType } = useCompany()
  const isManufaktur = activeBusinessType === 'manufaktur'
  const [products, setProducts] = useState([])
  const [q, setQ] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ code: '', name: '', unit: 'pcs', sale_price: 0, item_type: 'finished_goods' })
  ...
  async function load() {
    const { data, error } = await supabase
      .from('products')
      .select('id, code, name, unit, sale_price, qty_on_hand, avg_unit_cost, item_type')
  ...
  async function handleAdd(e) {
    ...
    setForm({ code: '', name: '', unit: 'pcs', sale_price: 0, item_type: 'finished_goods' })
    setShowForm(false)
    load()
  }
    ...
    <div className="p-6">
      ...
      {canEdit && showForm && (
        <form className="section inline-form" onSubmit={handleAdd}>
          ...
          {isManufaktur && (
            <div className="field">
              <label>{t('produk.itemType')}</label>
              <select value={form.item_type} onChange={(e) => setForm({ ...form, item_type: e.target.value })}>
                {ITEM_TYPES.map((it) => <option key={it} value={it}>{t(`produk.itemType_${it}`)}</option>)}
              </select>
            </div>
          )}
      ...
      <table className="tbl">
        <thead>
          <tr>
            ...
            {isManufaktur && <th>{t('produk.colItemType')}</th>}
        ...
        <tbody>
          {filtered.map((p) => (
            <Fragment key={p.id}>
              <tr>
                ...
                <td>{p.name}</td>
                <td>{p.unit}</td>
                {isManufaktur && <td><span className="badge pending">{t(`produk.itemType_${p.item_type}`)}</span></td>}
              ...
              {expandedId === p.id && (
                <tr>
                  <td colSpan={isManufaktur ? 9 : 8} style={{ background: 'var(--surface-alt)', padding: 14 }}>
          ...
          {!filtered.length && <tr><td colSpan={isManufaktur ? 9 : 8} className="empty">{t('produk.noProducts')}</td></tr>}
        </tbody>
      </table>
    </div>
  )
