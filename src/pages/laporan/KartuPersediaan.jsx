...
export default function KartuPersediaan() {
  ...
  const exportColumns = [
    ...
    { key: 'note', headerId: tid('kartuPersediaan.colNote'), headerEn: ten('kartuPersediaan.colNote'), align: 'left' },
  ...
  const exportRows = rows.map((r) => ({
    ...
    qty: r.qty, unitCost: r.unit_cost, totalCost: r.total_cost, runningQty: r.running_qty, runningValue: r.running_value, note: r.note || '-',
  }))
  const exportConfig = {
    companyName,
        ...
        <table className="tbl">
          <thead>
            <tr>
              ...
              <th>{t('kartuPersediaan.colNote')}</th>
          ...
          <tbody>
            ...
            {!rows.length && <tr><td colSpan={8} className="empty">{t('kartuPersediaan.noData')}</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
