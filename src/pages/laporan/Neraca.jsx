...
export default function Neraca() {
  ...
  const labaTahunBerjalan = pendapatanBeban.reduce((s, r) => {
    ...
    // netAmount() sudah diarahkan sesuai normal_balance akun itu sendiri (positif =
    // saldo bertambah ke arah normalnya). Dampaknya ke laba tergantung KOMBINASI
    // kategori + arah normalnya — bukan cuma kategori doang. Ini nangkep akun
    // kontra (misal "Retur Penjualan" = kategori pendapatan tapi normal_balance
    // debit, jadi harusnya MENGURANGI laba, bukan menambah).
    if (r.category === 'pendapatan') {
      return s + (r.normal_balance === 'kredit' ? amt : -amt)
    }
    // beban_pokok / beban_operasional — normalnya debit (mengurangi laba).
    // Kalau ada akun kontra-beban (normal_balance kredit, jarang tapi mungkin),
    // itu justru menambah laba.
    return s + (r.normal_balance === 'debit' ? -amt : amt)
  }, 0)
      ...
      <div className="p-6">
        ...
        <div className="filters">
          ...
          <select value={month} onChange={(e) => setMonth(Number(e.target.value))}>
            {(i18n.language === 'id' ? MONTH_NAMES_ID : MONTH_NAMES_EN).map((m, idx) => (
