...
export default function PerubahanModal() {
  ...
  const [modalLainnya, setModalLainnya] = useState(0)
  ...
  async function load() {
    ...
    const { data: lainnyaData, error: lainnyaErr } = await supabase.rpc('get_modal_other_movement', {
      p_company_id: companyId, p_start: period.start, p_end: period.end + 'T23:59:59', ...dim.rpcParams,
    })
    if (lainnyaErr) console.error(lainnyaErr)
    setModalLainnya(Number(lainnyaData) || 0)
  ...
  const modalAkhirDihitung = modalAwal + setoran - penarikan + modalLainnya + labaBersih
  ...
  const exportRows = [
    ...
    ...(modalLainnya !== 0 ? [{ desc: `${tid('perubahanModal.otherCapitalMovement')} / ${ten('perubahanModal.otherCapitalMovement')}`, amount: modalLainnya }] : []),
    ...
    <div className="p-6">
      ...
      <table className="tbl">
        <tbody>
          ...
          {modalLainnya !== 0 && (
            <tr><td style={{ paddingLeft: 24 }}>{t('perubahanModal.otherCapitalMovement')}</td><td className="num">{fmt(modalLainnya)}</td></tr>
          )}
