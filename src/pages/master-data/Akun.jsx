...
export default function Akun() {
  ...
  async function handleAdd(e) {
    ...
    setSaving(true)
    if (editingId) {
      const { error } = await supabase.from('accounts').update(form).eq('id', editingId)
      setSaving(false)
  ...
  function openEdit(a) {
    setEditingId(a.id)
    setForm({ code: a.code, name: a.name, category: a.category, normal_balance: a.normal_balance, description: a.description || '' })
    setShowForm(true)
  }
      ...
      <div className="p-6">
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            {canEdit && (
              <>
                ...
                <button className="btn-primary" onClick={() => { if (showForm) { closeForm() } else { setEditingId(null); setShowForm(true) } }}>
        ...
        {canEdit && showForm && (
          <form className="section inline-form" onSubmit={handleAdd}>
            ...
            <button className="btn-primary" disabled={saving}>{saving ? t('common.saving') : (editingId ? t('common.save') : t('akun.save'))}</button>
            {editingId && <button type="button" className="btn-secondary" onClick={closeForm}>{t('common.cancel')}</button>}
          </form>
        )}
                ...
                {canEdit && (
                  <td>
                    <button
                      ...
                      title={a.is_locked ? t('akun.lockedHint') : t('common.edit')}
                      style={{ marginRight: 6 }}
                    >
                      <Pencil size={14} />
                    </button>
