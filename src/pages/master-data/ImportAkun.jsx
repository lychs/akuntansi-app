...
export default function ImportAkun() {
  ...
  async function handleImport() {
    ...
    setLog(newLog)
    setImporting(false)
    // Kalau semua baris sukses, bersihin preview biar tombol "Upload" ilang
    // (gak bisa ke-klik lagi buat data yang sama). Kalau ada yang gagal,
    // preview cuma nyisain baris yang gagal itu — klik Upload lagi cuma
    // ngulang baris yang emang belum berhasil, bukan yang udah sukses.
    setRows((prev) => prev.filter((_, i) => newLog[i] && !newLog[i].ok))
  }

  if (activePlan === 'free') {
    return <div className="p-6"><PlanLockNotice featureName={t('sidebar.importAccounts')} /></div>
  }
