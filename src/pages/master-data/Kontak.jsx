      ...
      <div className="p-6">
        ...
        {canEdit && showForm && (
          <form className="section inline-form" onSubmit={handleAdd}>
            ...
            <button className="btn-primary" disabled={saving}>{saving ? t('common.saving') : (editingId ? t('common.save') : t('kontak.save'))}</button>
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id}>
                ...
                {canEdit && (
                  <td>
                    <button className="btn-icon" onClick={() => openEdit(c)} title={t('common.edit')} style={{ marginRight: 6 }}>
                      <Pencil size={14} />
                    </button>
